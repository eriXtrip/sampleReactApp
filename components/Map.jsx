...
function CandyMap({ stops = 20, cols = 5, progress = 0, accentColor = '#48cae4', style }) {
  ...
  const renderNodes = () => {
    ...
    for (let i = 0; i < stops; i++) {
      ...
      const fill = candyPalette[i % candyPalette.length];
      nodes.push(
        <React.Fragment key={`node-${i}`}>
          {/* Glow under candy for trail feel */}
          <Circle cx={x} cy={y} r={r + 8} fill={accentColor} opacity={0.08} />
          {/* Candy base */}
          ...
          {/* Number */}
          <SvgText x={x} y={y + metrics.fontSize / 3} fontSize={metrics.fontSize} fill="#1f2937" fontWeight="700" textAnchor="middle">
            {label}
          </SvgText>
        </React.Fragment>
      );
  ...
  const renderMarker = () => {
    ...
    return (
      <>
        <Circle cx={x} cy={y} r={r + 10} fill={accentColor} opacity={0.16} />
        <Circle cx={x} cy={y} r={r + 4} stroke={accentColor} strokeWidth={4} fill="none" />
      </>
    );
  };
  ...
  const renderLabels = () => {
    ...
    return (
      <>
        <SvgText x={start.x} y={start.y + (metrics.radius + f + 2)} fontSize={f} fill={theme.text} textAnchor="middle">Start</SvgText>
        <SvgText x={goal.x} y={goal.y - (metrics.radius + 6)} fontSize={f} fill={theme.text} textAnchor="middle">Goal</SvgText>
      </>
    );
  };
